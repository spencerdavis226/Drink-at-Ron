...
- Card scene artwork resolves through `src/presentation/artwork.ts` (`resolveArtwork`), shared by render, preload and `scripts/validate-content.ts`. Add an illustration as a `public/art` file plus a registry entry (path + `scene` + `fit`); never branch on a card ID in `CardFace`. Bundled frame surfaces live only in `src/presentation/frame-surfaces.ts`; do not restore an eager glob of `presentation/art`.
