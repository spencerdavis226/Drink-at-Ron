...
- The full-screen `@3d-dice/dice-box-threejs` overlay is the default dice renderer (`src/components/FullScreenDice.tsx`, adapter `src/presentation/dice/library.ts`); it is lazy-loaded and must be fed predetermined results from `src/game/dice.ts`. The catalog has four opt-in packs: the generated main deck (`src/content/{sample,classics,standard-expansion}.ts`: 117 cards), the supplied Sheet1 House deck (`src/content/custom.ts`: 102 cards, restored verbatim from the raw sheet kept at `reference/Drink at Ron - Sheet1.csv`), `VIP night` (`src/content/vip.ts` + the four `vip.sheet-*` entries in `custom.ts`: 16 cards), and the Pokémon board-sheet translation (`src/content/pokemon.ts`: 134 cards, morphed from `reference/pokemon_board_spaces.json` with the source consequences kept as written). Cards are authored as title + rules + category + optional dice, with no illustration text. Every dice card must resolve to one exact computed instruction (outcomes by total, optional `step` for odds/evens, `doubles`, and `{total}`/`{first}`…`{fourth}` placeholders); keep legacy `instruction` prose resolution for old saves. Keep engine/state contracts and saved-order behavior unchanged. Do not reintroduce a second renderer, and do not change per-body damping/sleep between the library's pre-simulation and its replay — that diverges the forced face.
