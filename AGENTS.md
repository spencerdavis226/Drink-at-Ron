...
- The dice renderer is deterministic CSS 3D (`src/components/FullScreenDice.tsx`, `src/presentation/dice/DiceRoll.tsx`, orientation math in `src/presentation/dice/geometry.ts`), driven by engine-owned values from `src/game/dice.ts`. No WebGL and no physics engine — do not add one. The animation blends to an exact landing per value, so it cannot plop or land on the wrong face. Provisional dice cards live in the `dice` pack; keep the validated 30-card Core unchanged.
