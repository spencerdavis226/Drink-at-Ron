...
Choose anyone to draw first. Pass the device clockwise unless a card names the next drawer. The drawer starts a category or challenge. Complete its activity before dismissing the card. Category rounds end on the first repeat or blank; no stopwatch or debate about how long someone hesitated. Every card makes its actor explicit: the drawing player, the whole table, or a named subset. A lasting rule runs either for the rest of the game or until the drawing player's next turn — never until the next card — with explicit consequences in the card text. No rule requires app bookkeeping.

## Current content

Four opt-in packs. `core` (117 cards) is the generated main deck: the supplied sample set (34), trimmed classics (45), and the trimmed dice expansion (38). `house` (102 cards) is the supplied Sheet1 restored verbatim in `src/content/custom.ts`. `VIP night` (16 cards) covers birthdays and celebrations. `pokemon` (134 cards, `src/content/pokemon.ts`) translates the supplied Kanto/Johto/Hoenn board sheets into the shared-deck format: gyms became leader battles, board movement and lost turns became pours or one-shot dice, and untitled spaces are named after the Pokémon in them. Source consequences are kept as written — shots, chugs, gendered splits, mixed drinks and rough dares are not softened. Any combination of packs can be selected; at least one is required. The four packs comprise 78 sip, 69 group, 30 category, 136 challenge, and 56 rule cards, 84 of them dice cards. This distribution is an editorial starting point, not validated balance.
