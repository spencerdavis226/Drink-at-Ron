...
The standard deck is the always-included `core` pack: the supplied 40-card sample collection plus 65 classic / King's Cup basics (`src/content/classics.ts`), for 105 cards — 17 sip, 24 group, 11 category, 35 challenge, 18 rule, with 20 cards carrying a dice definition. `VIP night` adds a 12-card themed pack (3 sip, 3 group, 2 category, 2 challenge, 2 rule). This distribution is supplied sample material, not validated balance. All cards are equally likely per shuffle cycle; a deck longer than the pool repeats cards only after exhausting it, and the cycle boundary never repeats immediately when the pool has multiple cards. Classic rules are adapted to the single sip unit from public King's Cup / party-deck write-ups; traditional speed-drinking and forced-consumption rules are not carried over.
...
Use the workshop to inspect every card at 320px and enlarged text. Play a Short (30) game with a real group, then an Infinite run through two shuffle cycles. Long (60) may cross a cycle boundary with Core alone; that is intentional. Record explanations, passes, unclear endings, and repetition before changing the collection. Do not introduce weights or pacing rules without observed need. Lock copy and IDs before generating the remaining individual illustrations.
