...
Use the workshop to inspect every card at 320px and enlarged text. Play a 40-card game with a real group, then a short Endless run. Record explanations, passes, unclear endings, and repetition before changing the collection. Do not introduce weights or pacing rules without observed need. Lock copy and IDs before generating the remaining individual illustrations.
