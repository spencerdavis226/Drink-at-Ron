...
The original checkout contains separate uncommitted portrait-gate/menu/test changes. They remain there unchanged. The 250-card expansion is prepared in an isolated release worktree with the preceding committed game work. No merge or publication yet.

**Next assigned task:** run the complete CI suite on the isolated release candidate, inspect 320/390px rule fit for all new cards, and resolve any reproducible failure before publishing. Then review the generated card sheet with the user and run a real group playtest; revise repetitive, unclear, or flat copy before calling the content final. Preserve the existing portrait-gate WIP separately.
