...
## Current handoff — 250-card Core, custom sheet, and table-readable type (2026-09-25)

User authorized validation, push, merge, and publication to GitHub Pages. The release is in isolated worktree `/Users/spencer/Documents/Dev/Drink-at-Ron-release-250` on `codex/release-250`; the original `codex/finish-v1` checkout and its separate uncommitted portrait/menu work remain untouched. PR #2 is https://github.com/spencerdavis226/Drink-at-Ron/pull/2. User found the repeated every-card sweeps too slow and requested a shorter publication gate. The normal PR/main workflow now uses a 14-case Chromium/WebKit release smoke suite plus unit, build/budget, and update checks; exhaustive browser/workshop sweeps are opt-in via manual `full_validation`. Do not merge until the new scoped gate passes, then verify the main deployment and live Pages URL.

Core has exactly **250 standard cards** (40 supplied sample, 65 classics, 145 researched additions). Rule text is **26px on phone cards, scaling to 34px on wide cards**, with enlarged mode at 34–42px. Authored rules target 90 characters / 18 words, and the build rejects above 120 characters / 24 words. The 2:3 front, pure engine, session schema v2/v1 migration, and storage key are unchanged. The pre-CSV release commit `f6b63ec` fixed two WebKit test timing races; its GitHub CI was still running when the CSV arrived.

Imported the supplied `Drink at Ron - Sheet1.csv` as data: **103 nonblank Main rows** in optional `house` pack (`Ron’s house cards`) and **4 VIP rows** appended to `VIP night`, for **369 catalog cards across 3 packs**. Main row 75 is blank. Main row 106 has title “Debate” but no description, so its released rule is a silly debate and vote. Stable IDs encode 1-based source rows; duplicate source headings remain distinct cards. Copy is adapted to the established single-sip/no-speed-drinking game rule and the larger-text guardrails. The sheet's text was never executed as instructions. `docs/CARD_REVIEW.md` and `docs/card-review.csv` regenerate from the shipping catalog for line-by-line review. The source CSV remains outside Git; review the edited copy against it. The custom pack is opt-in so Core remains at the user’s 250-card target.

Changed in this pass: `src/content/custom.ts`, `catalog.ts`, `vip.ts`, `imprint.ts`, `public/art/packs/house.svg`, composition/VIP tests, authoring/design/playtest/README guidance, generated review sheets, and this handoff. Initial verification: 91 unit tests passed; a Pages-path production build passed at 369 cards / 3 packs, 2610 KiB runtime, 96.5 KiB initial and 147.3 KiB lazy gzip JavaScript, within unchanged budgets. GitHub run `36209819372` passed unit, build, full Chromium production E2E, full WebKit production E2E, and the two-build update check on `d63e600`; the user asked to cancel it during the lengthy Chromium workshop sweep. A separate local WebKit small-phone every-card sweep failed at `house.sheet-096` because the preview iframe did not report ready within 15 seconds under extreme Mac load; the selected card was visible in the failure snapshot, and no card geometry assertion failed, but that sweep is **not green**. The new quick PR gate is pending. Physical iPhone/iPad and group playtesting remain separate unverified gates per `docs/DEVICE_CHECKLIST.md` and `docs/PLAYTEST.md`; do not describe them as passed from emulation.

**Next task:** validate the new quick release gate, push it to PR #2, wait for its CI result, merge the authorized PR, and verify GitHub Pages live assets, service worker, and release ID. Update this handoff with the final commit, checks, deployment, and remaining device/group gates.
