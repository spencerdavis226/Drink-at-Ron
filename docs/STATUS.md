...
Release candidate: the 250-card expansion is committed as `766c1bf` on `codex/release-250`, with production layout sampling in `79b7312`. The original `codex/finish-v1` checkout and its separate uncommitted portrait-gate/menu/test changes remain untouched. `npm ci`, the Pages build/budgets, the generated card-review drift check, and all 90 unit tests passed in the isolated release worktree. GitHub CI passed unit, build, and Chromium E2E for `79b7312`; WebKit E2E found three motion-test timing races. The revised tests attach animation listeners before activation and wait for the completed flip transform. The focused three-test WebKit rerun passed locally. Full CI, merge, publication, and live Pages verification are still pending as of this handoff edit.
