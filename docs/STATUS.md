...
Changed in this pass: `src/content/custom.ts`, `catalog.ts`, `vip.ts`, `imprint.ts`, `public/art/packs/house.svg`, composition/VIP tests, authoring/design/playtest/README guidance, generated review sheets, and this handoff. Initial verification: 91 unit tests passed; a production build passed at 369 cards / 3 packs, 2609 KiB runtime, 96.5 KiB initial and 147.3 KiB lazy gzip JavaScript, within unchanged budgets. Browser/workshop/update suites and final CI are still pending for the CSV addition. Physical iPhone/iPad and group playtesting remain separate unverified gates per `docs/DEVICE_CHECKLIST.md` and `docs/PLAYTEST.md`; do not describe them as passed from emulation.

**Next task:** finish content/layout validation; commit and push the custom-card change to PR #2; wait for full CI; merge the authorized PR; verify GitHub Pages live assets, service worker, and release ID. Update this handoff with the final commit, checks, deployment, and remaining device/group gates.
