...
## Current handoff — 250-card standard Core (2026-09-25)

User confirmed the target is **about 250 total standard Core cards**, not 250 additional cards. Added 145 original cards in `src/content/standard-expansion.ts`, making Core exactly **250 cards** and the full catalog **262 cards** with VIP. The new set has 30 sip, 30 group, 30 category, 40 challenge, and 15 rule cards; total Core distribution is 47 / 54 / 41 / 75 / 33. Dice remain at 20. The one-card format and active-session snapshots are unchanged. Distinct visual motifs are authored as illustration briefs; the retained imprint system uses existing category icon slugs, so its lazy sprite does not grow. Updated Core membership, authoring/design/README/playtest guidance, composition tests, and regenerated `docs/CARD_REVIEW.md` and `docs/card-review.csv` for copy review.

Research: [King's Cup printable guide](https://kingscupgame.com/wp-content/uploads/2026/01/Kings-Cup-Rules-Printable-Final.pdf) and the [King's Cup game rules](https://store.steampowered.com/app/1506610/) confirmed the familiar give/take, group, category, question, and temporary-rule families. The [adult icebreaker guide](https://www.socialworkerstoolbox.com/wp-content/uploads/2023/02/Ice-breakers-for-children-and-adults.pdf) supplied format ideas such as word association, charades, storytelling, and table choices. All new wording is original and adapted to this game's single-card flow, short rules, optional sips, and no app bookkeeping. Research informed the families, not a copied commercial deck.

Verification: `npm test` **90 passed**. `CI=1 BASE_PATH=/Drink-at-Ron/ npm run build` passed: 262 cards / 2 packs / 41 icons; **2594 KiB runtime, 93.4 KiB initial + 147.3 KiB lazy gzip JS**, under unchanged budgets. The authored-title sweep passed **2/2 in each of Chromium and WebKit** when run independently, checking all 262 titles at three card widths and normal/enlarged text. The complete Chromium small-phone geometry sweep passed **1/1** independently. Full workshop and production browser suites are **not green**: full workshop with two workers had 7 passed / 5 failed / 10 not run before interruption (title timeouts and preview readiness failures); the serial rerun timed out on the title sweep and was interrupted (1 failed / 1 interrupted / 19 not run). The focused production layout run had 6 passed, 2 failed, 1 flaky, 1 interrupted, 4 not run before interruption; failures were navigation/font readiness timeouts. The host reached load averages above 300 with unrelated macOS services consuming CPU. This is an environment timing diagnosis, **not proof** that all new cards fit or that the full suites pass. Do not increase per-preview waits or weaken geometry checks on that basis. The title sweep's whole-test budget was raised to 120s because six catalog passes now cover 262 cards; it still timed out under the host load. `git diff --check` passed before the final handoff edit. Physical iOS and group playtesting remain open.

Release candidate: the 250-card expansion is committed as `766c1bf` on `codex/release-250`, with production layout sampling in `79b7312`. The original `codex/finish-v1` checkout and its separate uncommitted portrait-gate/menu/test changes remain untouched. `npm ci`, the Pages build/budgets, the generated card-review drift check, and all 90 unit tests passed in the isolated release worktree. GitHub CI passed unit, build, and Chromium E2E for `9e3e553`; WebKit E2E still found two motion-test timeouts because WebKit can remove the phase animation before dispatching `animationend`. The tests now seek the active CSS animation to its authored landing frame and compare it with the resting transform. Ten focused WebKit repetitions passed locally with two workers. Full CI, merge, publication, and live Pages verification are still pending as of this handoff edit.

**Next assigned task:** run the complete CI suite on the isolated release candidate, including workshop card fit at phone widths and the two-build update flow. Resolve reproducible failures, merge and publish the authorized release, then verify the live Pages release ID and offline assets. Physical iOS checks and a real group playtest remain separate follow-ups; use the generated card sheet to revise repetitive, unclear, or flat copy before calling the content final. Preserve the existing portrait-gate WIP separately.
