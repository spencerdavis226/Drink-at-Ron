...
Status: continuous-frame revision awaiting user visual approval. These are real browser renders, not generated UI mockups. Generated images supply only the materials and one illustration; all titles and instructions are live DOM text.
...
| --- | --- | --- |
| Short instruction | [A little cheers](continuous-chromium-phone-cheers.png) | [A little cheers](continuous-chromium-ipad-cheers.png) |
| Longer category | [Wild company](continuous-chromium-phone-animals.png) | [Wild company](continuous-chromium-ipad-animals.png) |
| Temporary rule | [Left-handed magic](continuous-chromium-phone-left.png) | [Left-handed magic](continuous-chromium-ipad-left.png) |
...
## Current direction: continuous perimeter and goofy painted fantasy

The latest study replaces the stacked frames with one shared outer perimeter and a bracketed teal title band. The mouse is superseded by an independently designed enchanted suit of copper armor making a proud toast. Keep the rich painted fantasy-card rendering; Heroes of Barcadia and Munchkin inform absurd situations and visual humor, not copied characters or graphic assets.

The table above links the current continuous-frame captures. Earlier named captures remain historical. The current source/export script is `scripts/front-continuous.ts`; exact generation and correction prompts are recorded in ART_DIRECTION.md. Production promotion remains pending approval. Latest verification: 36 unit tests, production build/budgets, and both browser engines' workshop tests (all 30 cards, five sizes, normal/enlarged text) passed. Physical-device performance remains unverified.
